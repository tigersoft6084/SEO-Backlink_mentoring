import { PRENSALINK_API_URL } from '@/globals/globalURLs.ts';
...
export const getTokenForPrensalink = async (): Promise<string | null> => {
  ...
  const credentials = await getCredentialsForMarketplaces();
    ...
    for (const credential of credentials) {
        ...
        const hasPrensalinkTarget = credential.websiteTarget.some((target: { value: string }) => target.value === 'Prensalink');
