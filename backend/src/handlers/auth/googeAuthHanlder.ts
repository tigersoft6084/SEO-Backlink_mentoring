...
import { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI } from "@/config/apiConfig.ts";
...
export const googleAuthHandler = async (req: PayloadRequest) : Promise<Response> => {
    ...
    try {
        ...
        if (loggedInUser) {
            ...
            const frontendUrl = `http://localhost:1212/auth/google/callback?userData=${encodeURIComponent(JSON.stringify(loggedInUser))}`;
